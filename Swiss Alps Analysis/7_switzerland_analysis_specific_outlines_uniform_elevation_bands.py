...
for src, group in scatter_df.groupby('source'):
    ...
    plt.plot(weighted_means, rel_elevs, marker='.',linewidth=1, label=label,markersize=5    , color=colors.get(src, 'gray'))
    if src == 's2':
        min_x = min(weighted_means)
        shifted = [x - min_x for x in weighted_means]
        plt.plot(shifted, rel_elevs, marker='.', linewidth=1, linestyle='--',
                 label="s2 shifted", markersize=5, color=colors.get(src, 'gray'))
