...
# what this code does:
# Visualize  glacier DEMs with elevation bands and overlay debris cover from multiple sources (LX, SAM, S2, SGI) for selected glaciers in Switzerland, using RGI7 outlines as a reference
# - select rgi_id's to plot
# - Masks the DEM to the glacier extent using rasterio.mask to extract elevation data.
# - Calculates elevation bands (num_bands = 20) based on RGI7 glacier DEM:
# - Rasterizes debris polygons to the same shape as the glacier DEM: 1 = Debris, NaN = non-debris
# - Plots RGI7 glacier extent as a yellow background.
# - Plots source glacier DEM in blue hues.
# - Overlays elevation band contours (black lines).
# - Overlays debris cover in brown.
...
for idx, rgi_id in enumerate(rgi_ids_to_plot):
    ...
    cbar = fig.colorbar(im_glacier, ax=axes, orientation='horizontal', fraction=0.05, pad=0.1, label="Elevation [m]") 
    cbar.ax.yaxis.label.set_size(20)
    cbar.ax.tick_params(labelsize=14)
    fig.suptitle(f"Glacier DEM with Elevation Bins overlaid with Debris Cover\n{sgi_name} ({rgi_id})", font = 'serif', fontsize=19)
