...
# - loops through selected RGI 7.0 regions

# - reprojects DEM to region-specific projection
# - loops over each unique glacier ID in datasets

# - Loop over each glacier ID and source in the debris data:
#       - Select the correct outline layer (RGI7 for s2/lx, SAM for sam).
#       - Mask the DEM to the glacier outline (for analysis) and to the RGI7 outline (for elevation bins).
#       - Convert nodata/zero to NaN
#       - Rasterize the glacier polygon to count total glacier pixels.
#       - Compute elevation bands by normalizing DEM values into 1–20 bins.
#       - Add empty bands columns to the debris subset.
#       - For each debris polygon, rasterize its mask and, for each band, calculate: debris pixels/total pixels
#       - Counts NAN values where bands are missing (due to glacier outlines being smaller than RGI 7 outlines (in sam))
#       - Append the annotated debris subset

# - Report how many glaciers had any NaN band values and print a table of NaN counts by source & band.
# - Concatenate all per‑glacier subsets into a final GeoDataFrame 
# - Create a 3‑panel plot:
#       - Scatter of pixel‐ratio vs. relative elevation by source.
#       - Mean ratio per band (with the S2 curve shifted to zero).
#       - Weighted mean ratio per band, weighted by glacier pixel area (with S2 shifted).
# - plot max difference band distribution - with s2-shift from mean plot applied to s2
# - combine RGI 7.0 outlines + all regions' file with elevation band information and save to all_together director
...
if combined_rgi7_list:
    ...
    print(" No RGI7 files found to combine.")
...
if all_debris_gdfs:
    ...
    print("No debris cover files found to combine.")
