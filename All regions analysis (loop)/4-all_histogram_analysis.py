...
for region in regions:
    ...
    axes[0].set_title(f"LX ({year_lx}) → SAM ({year_sam})", fontsize=17)
    axes[0].set_xlabel("Fractional Debris Cover Change",fontsize=16.5)
    axes[0].set_ylabel("Frequency",fontsize=16.5)
    axes[0].set_ylim(0, int(np.max(counts_ls)) * 1.05)
    ...
    axes[1].set_title(f"SAM ({year_sam}) → S2 ({year_s2})", fontsize=17)
    axes[1].set_xlabel("Fractional Debris Cover Change")
    axes[1].set_ylabel("Frequency")
    axes[1].set_ylim(0, int(np.max(counts_ss)) * 1.05)
    ...
    axes[2].set_title(f"LX ({year_lx}) → S2 ({year_s2})", fontsize=17)
    axes[2].set_xlabel("Fractional Debris Cover Change",fontsize=16.5)
    axes[2].set_ylabel("Frequency",fontsize=16.5)
    axes[2].set_ylim(0, int(np.max(counts_ls2)) * 1.05)
