...
# What this code does:
# - loop through RGI 7.0 regions
# - reprojects all datasets to region-specific crs
# - Uniform renaming of attributes
# - extracts the img_year from "img_time"
# - keeps only glaciers > 2km^2
# - Checks for duplicates, visualizes them, handles them:
#       - If duplicate geometries overlap among themselves (intersection > 0), keep only the feature with the largest area_col.
#       - Else if all duplicates overlap the corresponding rgi_glacier by at least 90% fraction of their own area,
#         merge their geometries (union) and sum numeric attributes; other attributes taken from the largest-area feature.
#       - Otherwise omit the entire group.
# - saves "clean" datasets (no duplicates, same crs etc.)
# - creates new "combined" dataset for later analysis with new column source and only following columns: 'geometry', 'area_km2', 'debris_cover_ratio', 'img_time', 'img_year', 'rgi_id'
# combines combined files of the selected RGI regions and saves into "all_together" - directory - this is used for the aggregated analysis
...
if all_combined:
    ...
    print(f"All regions combined and saved to: {output_path}")
else:
